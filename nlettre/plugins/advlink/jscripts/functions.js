...
function preinit() {
	...
	/* Import external list url javascript : en commentaire richir
...
function init() {
	...
	if (action == "update") {
		...
		kr = href.indexOf("cms/"); kr = kr != -1 ? href.substr(kr) : href; //--- richir
		selectByValue(formObj, 'linklisthref', kr);
...
function getLinkListHTML(elm_id, target_form_element, onchange_func) { //--- richir : traiter tinyMCELinkList
	if (typeof(tinyMCEPopup.windowOpener.vatimcelilink) == "undefined" || tinyMCEPopup.windowOpener.vatimcelilink.length == 0)
		return "";
	...
	j = tinyMCEPopup.windowOpener.vatimcelilink.length;
	for (var i=0; i<j; i++) {
		k = tinyMCEPopup.windowOpener.vatimcelilink[i];
		html += '<option value="cms/' + k + '.php">' + k + '</option>';
	}
	html += '</select>';
