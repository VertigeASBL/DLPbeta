<?
//---------- Envoi de newsletter / DLP Concours (attention "letrEnvoi" => "letrEnvConc") ----------
...
function obtenirconcours(&$tab) {
	global $conc_sel;
/*	voir squelettes/rubrique=95.html, agenda/spip/conc_public.php
	Obtenir les concours EN COURS
*/
	...
	$reponse = mysql_query("SELECT * FROM $table_ag_conc_fiches WHERE lots_conc LIKE '$public_cible_like' AND cloture_conc > $limit_afficher AND flags_conc LIKE '%actif%'".($conc_sel ? ' AND selection_conc=1' : '').' ORDER BY id_conc DESC');
	$tab = '';
	while ($donnees = mysql_fetch_array($reponse))
	{
		...
		if (isset($donnees['pic_conc']) AND $donnees['pic_conc'] == 'set' )
		{
			if (isset($donnees['event_dlp_conc']) AND $donnees['event_dlp_conc'] != 0 )
				$chn = '<a href="http://www.demandezleprogramme.be/-Detail-agenda-?id_event=' . $donnees['event_dlp_conc'] . '" style="text-decoration:none;">';
			else
				$chn = '';
			$tab.= '<span class="conc_photo">'.$chn.'<img src="http://www.demandezleprogramme.be/agenda/' . $folder_vignettes_concours . 'vi_conc_' . $id_conc . '_1.jpg" style="border:1px solid #000000; background-color:#FFFFFF; padding:1px;" alt="" />'.($chn ? '</a>' : '').'</span>'."\n";
		}

		// TITRE ---$tab.= '<div align="right">(réf ' . $id_conc . ')</div><br />' ;
		$nom_event_conc = stripslashes ($donnees['nom_event_conc']) ;
		$tab.= '<div class="bloc_texte">'."\n".'<span class="titre_conc">' . $nom_event_conc .'</span><br /><br />'."\n";
		...
		if (isset($donnees['event_dlp_conc']) AND $donnees['event_dlp_conc'] != 0 )
		{
			$tab.= '<br /><br /><a href="http://www.demandezleprogramme.be/-Detail-agenda-?id_event=' . $donnees['event_dlp_conc'] . '"><img src="http://www.demandezleprogramme.be/agenda/design_pics/' . $bouton_tout_savoir . '" align="top" alt="" /></a>'."\n";
		}
		...
		{
			$tab.= '<br /><br /> <a href="' . $donnees['lien_externe_conc'] . '"> &gt;&gt; Voir les infos sur le site de l\'événement</a>'."\n";
		}
		...
		if ($cloture == 'fini')
		{
			$tab.= '<br /><br /><span class="date_cloture">Concours Clôturé</span>'."\n"; 
		}
		else
		{
			$tab.= '<br /><br /><span class="date_cloture">Date de cloture : le ' . $date_cloture_jour . ' ' . $NomDuMois[$date_cloture_mois+0] . ' ' . $date_cloture_annee . ' à ' . $date_cloture_heure .'h00 </span>'."\n"; 
		}
		...
		if (isset($donnees['lots_conc']) AND ($donnees['lots_conc'] != NULL))
		{
			...
			foreach ($var_lot_unserialized as $element_lot)
			{
				...
				{
					$tab.= '<li><img src="http://www.demandezleprogramme.be/squelettes/assets/puce2.jpg" alt="" style="vertical-align:middle;" />&nbsp;' . $element_lot['decription_lot'] ;
					...
					if ($date_actuelle <= $date_cloture)
					{
						$tab.= ' <a href="http://www.demandezleprogramme.be/' . $page_contenant_concours . '?id=' . $id_conc . '&amp;lot=' . $i_lot . '"><img src="http://www.demandezleprogramme.be/agenda/design_pics/conc_jouer.gif" align="top" alt="" /></a><br />';
					}
					else
					{
						$tab.= '<br />';
		...
		{
			$tab.='<br />ERREUR (Aucun lot pour ce concours)'."\n";
		}
		$tab.='</div>'."\n".'<div style="clear: both; font-size:4px; line-height:4px;">&nbsp;</div>'."\n".'</div> <br />'."\n\n";
	}
}
	...
	**********************************************/
	function obtenirmessage() {
		global $modl, $nlsuj, $nlang, $nltext, $nlhtml, $tficid, $tfmcid, $tnfich, $lidelai, $linblot, $nbrenv, $alerter, $sql, $req, $data, $chn, $volume;
		...
		if ($nlid && $nlhtml) {
			...
			$sql = '';
			obtenirconcours($sql);
			$nlhtml = str_replace('###_CONCOURS_###', $sql, $nlhtml);
			$sql = '';
	...
	if ($oper == 'fini' || $oper == 'azero') {
		...
		if ($data = mysql_fetch_array($req, MYSQL_ASSOC)) {
			...
			if ($data['texte'] && $oper == 'fini') {
				...
				//--- Concours
				$data['html'] = '<hr /><b>----- Concours -----</b><hr />'.$data['html'];
	...
	********************************/
	if (! $oper) {
		...
		if ($data = mysql_fetch_array($req, MYSQL_ASSOC)) { //--- Poursuivre ou annuler ?
			...
			if ($data['modl'] != $modl || ! ($modl == 5 && ! $conc_sel || $modl == 7 && $conc_sel))
				$alerter .= '\nAttention, avec ce modèle, il faut poursuivre autrement';
			$oper = 'cours';
		}
		else {
			$lidelai = 30; $linblot = 10;
...
//--- prov	$nlamail = 'philippe@vertige.org';
			...
			if (! $alerter && ! ($modl == 5 && ! $conc_sel || $modl == 7 && $conc_sel))
				$alerter = 'Attention, il faut envoyer autrement une newsletter avec ce modèle';
		}
	}
...
<?
	...
	echo '<div class="divgauche"><img src="matos/puce.gif" alt="" /> Envoyer une newsletter DLP - Concours<a href="http://www.vertige.org/aidecms/aideCMS.php?apg=diffusion" target="waide"><img src="matos/aide.gif" class="ico" alt="" title="aide" /></a></div>',"\n";
	...
	if ($oper == 'stop')
		...
		echo '<div class="divcentre"><br /><input type="button" class="bouton" onclick="window.location.href=\'letrEnvConc.php?cmd=suivr&amp;tci=',$tci,'\';" value="Poursuivre" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrEnvConc.php?cmd=arret&amp;tci=',$tci,'\';" value="Arrêter" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrMenu.php?tci=',$tci,'\';" value="Retour" /></div>',"\n";
	}
	else {
		echo '<form id="iofo" action="letrEnvConc.php?tci=',$tci,'" method="post">',"\n";
