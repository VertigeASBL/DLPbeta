<?
//---------- Envoi de newsletter / critères supplémentaires : chercher "ncritr", "datnaiss" ----------
...
//$adrdusite = 'interview@demandezleprogramme.be';
	...
	**********************************************/
	function obtenirmessage() {
		global $modl, $nlsuj, $nlang, $nltext, $nlhtml, $tficid, $tfmcid, $tnfich, $lidelai, $linblot, $nbrenv, $alerter, $sql, $req, $data, $chn;
		...
		while ($data = mysql_fetch_array($req, MYSQL_ASSOC))
			if ($data['quoi'] == 'modele') {
				...
				if ($nlsuj = addslashes($data['sujet']))
					$vol += strlen($nlsuj) + 157;
				$nlang = $data['nmulti'];
				if ($nltext = addslashes($data['texte']))
					$vol += strlen($nltext) + 157;
				$nlhtml = $data['html'];
			}
	...
	********************************/
	if (! $oper) {
		...
		if ($data = mysql_fetch_array($req, MYSQL_ASSOC)) { //--- Poursuivre ou annuler ?
			...
			if ($data['modl'] != $modl || ! ($modl == 1 || $modl == 4 || $modl == 6 || $modl == 9))
			...
			if (! $alerter && ! ($modl == 1 || $modl == 4 || $modl == 6 || $modl == 9))
...
<?
	...
	echo '<div class="divgauche"><img src="matos/puce.gif" alt="" /> Envoyer une newsletter<a href="http://www.vertige.org/aidecms/aideCMS.php?apg=diffusion" target="waide"><img src="matos/aide.gif" class="ico" alt="" title="aide" /></a></div>',"\n";
	...
	if ($oper == 'stop')
		...
		echo '<div class="divcentre"><br /><input type="button" class="bouton" onclick="window.location.href=\'letrEnvoi.php?cmd=suivr&amp;tci=',$tci,'\';" value="Poursuivre" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrEnvoi.php?cmd=arret&amp;tci=',$tci,'\';" value="Arrêter" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrMenu.php?tci=',$tci,'\';" value="Retour" /></div>',"\n";
	}
	else {
		echo '<form id="iofo" action="letrEnvoi.php?tci=',$tci,'" method="post">',"\n";
