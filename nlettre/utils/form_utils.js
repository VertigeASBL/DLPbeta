...
function getColorPickerHTML(id, target_form_element) {
	...
	html += ' width="20" height="16" border="0" title="' + tinyMCE.getLang('lang_theme_colorpicker_title') + '"';
	html += ' class="mceButtonNormal" alt="' + tinyMCE.getLang('lang_theme_colorpicker_title') + '" /></a>'; //--- richir lang_
...
function getBrowserHTML(id, target_form_element, type, prefix) {
	if (id != "srcbrowser" || prefix != "advimage") return ""; //--- richir
	...
	html += ' onmouseout="this.className=\'mceButtonNormal\';"';
	html += ' onmousedown="this.className=\'mceButtonDown\';"';
	html += ' width="20" height="18" border="0" title="' + tinyMCE.getLang('lang_insert_imgsave_title') + '"';
	html += ' class="mceButtonNormal" alt="' + tinyMCE.getLang('lang_insert_imgsave_title') + '" /></a>'; //--- richir lang_
...
function addClassesToList(list_id, specific_option) {
	...
	if (styles) {
		...
		for (var i=0; i<stylesAr.length; i++) {
			if (stylesAr != "") {
				var vrichir=stylesAr[i].indexOf(" ");vrichir=vrichir!=-1?stylesAr[i].substring(0,vrichir):stylesAr[i];
				styleSelectElm.options[styleSelectElm.length] = new Option(stylesAr[i], vrichir); //--- richir
			}
		}
	} else {
