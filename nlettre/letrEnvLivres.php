<?
//---------- Envoi de newsletter / DLP les Livres (attention "letrEnvoi" => "letrEnvLivres") ----------
...
function obtenirlivres(&$chn) {
	/**** Récup le code généré par spip *****/
	$k = 0; $err = '';
	$fp = fsockopen('www.demandezleprogramme.be', 80, $k, $err, 10);
	if (! $fp)
		echo '--- Connexion impossible : ',$k,' : ',$err,' ---';
	else {
		//--- Envoyer la requête
		fputs($fp, 'GET /spip.php?page=livresnews HTTP/1.0'."\r\n");
		fputs($fp, 'HOST: www.demandezleprogramme.be'."\r\n");
		...
		while (! feof($fp)) {
			...
			if ($k)
				$chn .= $chn1;
			else if (strpos($chn1, '**V_COM**') !== false)
				$k = true;
		}
		fclose($fp);
	}
	...
	**********************************************/
	...
	function obtenirmessage() {
		...
		if ($nlid && $nlhtml) {
			...
			$sql = '';
			obtenirlivres($sql);
			$nlhtml = str_replace('###_LIVRES_###', $sql, $nlhtml);
			$sql = '';				
	...
	if ($oper == 'fini' || $oper == 'azero') {
		...
		if ($data = mysql_fetch_array($req, MYSQL_ASSOC)) {
			...
			if ($data['texte'] && $oper == 'fini') {
				...
				//--- le mois courant
				$data['html'] = '<hr /><b>----- Livres -----</b><hr />'.$data['html'];
...
<?
	...
	echo '<div class="divgauche"><img src="matos/puce.gif" alt="" /> Envoyer une newsletter DLP - Les livres<a href="http://www.vertige.org/aidecms/aideCMS.php?apg=diffusion" target="waide"><img src="matos/aide.gif" class="ico" alt="" title="aide" /></a></div>',"\n";
	...
	if ($oper == 'stop')
		...
		echo '<div class="divcentre"><br /><input type="button" class="bouton" onclick="window.location.href=\'letrEnvLivres.php?cmd=suivr&amp;tci=',$tci,'\';" value="Poursuivre" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrEnvLivres.php?cmd=arret&amp;tci=',$tci,'\';" value="Arrêter" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrMenu.php?tci=',$tci,'\';" value="Retour" /></div>',"\n";
	}
	else {
		echo '<form id="iofo" action="letrEnvLivres.php?tci=',$tci,'" method="post">',"\n";
