<?
//---------- Envoi de newsletter / DLP la Une / semaine (attention "letrEnvoi" => "letrEnvUneSem"), modèle 3 "DLP_une" ----------
...
for ($jj = -15; $jj <= 15; $jj++) {
	$tt = time() + $jj * 86400;
	$moisctxt = date('d-m-Y'   ,$tt);

	$g = date('w'    ,$tt); $g--; if ($g < 0) $g = 6;
	$k = $tt;
	if ($g < 3)
		$k -= 86400 * $g;
	else
		$k += 86400 * (7 - $g);
	$datdbu = date('Y-m-d', $k);
	$datfin = date('Y-m-d', $k + 86400 * 6);
echo ' --- ',$g,' --- ',$moisctxt,' --- ',$datdbu,' --- ',$datfin,' --- <br />';
}
------ */

/* ------------------------
	du lundi au mercredi : semaine courante
	du jeudi au dimanche : semaine prochaine
	semaine = du lundi au dimanche
	lu 0, ma 1, me 2, je 3, ve 4, sa 5, di 6
	$moisctxt = 'du '.$datdbu.' au '.$datfin;
*/
	$g = date('w'); $g--; if ($g < 0) $g = 6;
	$k = time();
	if ($g < 3)
		$k -= 86400 * $g;
	else
		$k += 86400 * (7 - $g);
	$datdbu = date('Y-m-d', $k);
	$datfin = date('Y-m-d', $k + 518400);
	$moisctxt = 'de la semaine';
...
function obtenirune(&$chn) {
	global $datdbu, $datfin, $genres;

	$sql = " WHERE date_event_debut<='$datfin' AND date_event_fin>='$datdbu' AND pic_event_1='set'";
...
//--- echo $sql,'<hr />';
	...
	while ($donnees = mysql_fetch_array($reponse, MYSQL_ASSOC)) {
		$k = $donnees['lieu_event'];
		$t_evn[$k][] = $donnees['id_event'];
		$t_dbu[$k][] = $donnees['date_event_debut'];
	}
/* ------------------------
	si avant datdbu, alors enlever les précédents, sinon enlever le précédent et les suivants
*/
	reset($t_evn);
	while (list($lieu) = each($t_evn)) {
		reset($t_evn[$lieu]);
		while (list($g) = each($t_evn[$lieu]))
			if ($t_evn[$lieu][$g])
				if ($t_dbu[$lieu][$g] < $datdbu)
					for ($k = $g, $k--; isset($t_evn[$lieu][$k]); $k--)
						$t_evn[$lieu][$k] = false;
				else {
					$k = $g; $k--;
					if (isset($t_dbu[$lieu][$k]))
						$t_evn[$lieu][$k] = false;
					for ($k = $g, $k++; isset($t_evn[$lieu][$k]); $k++)
						$t_evn[$lieu][$k] = false;
				}
...
while (list($lieu) = each($t_evn)) {
	reset($t_evn[$lieu]);
	while (list($g) = each($t_evn[$lieu])) {
		echo $lieu,' --- ',$g,' --- ',$t_dbu[$lieu][$g],' --- ',$t_evn[$lieu][$g],' --- <br />';
	}
}
echo '<hr />';
------ */
...
/* ------------------------
	conserver seulement 1 événement par lieu
*/
	reset($t_evn);
	while (list($lieu) = each($t_evn)) {
		reset($t_evn[$lieu]);
	...
	while ($donnees = mysql_fetch_array($reponse, MYSQL_ASSOC)) {
		...
		if ($t_evn[$donnees['lieu_event']] != $id_event)
			continue;
//--- $chn .= $id_event.' --- '.$donnees['lieu_event'].' --- '.$donnees['date_event_debut'].' --- '.$donnees['date_event_fin'].' --- <br />';
...
//--- echo '<hr />'; echo stripslashes($nlhtml); echo '<hr />';
		}
	}
...
<?
	...
	echo '<div class="divgauche"><img src="matos/puce.gif" alt="" /> Envoyer une newsletter DLP - La Une / semaine<a href="http://www.vertige.org/aidecms/aideCMS.php?apg=diffusion" target="waide"><img src="matos/aide.gif" class="ico" alt="" title="aide" /></a></div>',"\n";
	...
	if ($oper == 'stop')
		...
		echo '<div class="divcentre"><br /><input type="button" class="bouton" onclick="window.location.href=\'letrEnvUneSem.php?cmd=suivr&amp;tci=',$tci,'\';" value="Poursuivre" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrEnvUneSem.php?cmd=arret&amp;tci=',$tci,'\';" value="Arrêter" /> &nbsp; &nbsp; &nbsp; &nbsp; <input type="button" class="bouton" onclick="window.location.href=\'letrMenu.php?tci=',$tci,'\';" value="Retour" /></div>',"\n";
	}
	else {
		echo '<form id="iofo" action="letrEnvUneSem.php?tci=',$tci,'" method="post">',"\n";
