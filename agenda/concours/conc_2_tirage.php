...
<div class="menu_back">
<a href="../admin_agenda/conc_2_listing.php" >Listing des concours  </a> | 
<a href="../admin_agenda/concours_lire_historique.php">Historique des tirages de concours</a></div>
	...
	while ($donnees = mysql_fetch_array($reponse))
	{
		...
		foreach ($var_lot_unserialized as $element_lot)
		{
			echo '<br /> <br /> <div class="titre_turquoise" align="center"> :::::::::::::: Lot ' . $i_lot . ' :::::::::::::: <b>'
			 . str_pad($element_lot['nombre_places'], 3, "0", STR_PAD_LEFT) . '</b> unités pour le groupe "'
			 . $groupes_joueurs[$element_lot['groupe_joueur']] . '". Intilulé de ce lot : '
			 . $element_lot['decription_lot'] . '</div>' ;
	
							
			// Données pour le LIEU culturel
			$liste_infos_gagnants.= '<br /> <br /><div class="email_style_rubriques">'
			. $element_lot['decription_lot'] . '<br />
			Lieu de l\'événement : '. $adresse_conc . '</div> ';
	
	
			// Données pour e-mail joueurs
			$nom_event_pour_mail_joueur = $donnees ['nom_event_conc'] ;
			
			$descriptif_lot_pour_mail_joueur = $element_lot['txt_mail_lot'] ;
	
	
			// Données pour l'Admin
			$rapport_complet_concat.= '[Lot ' . $i_lot . '] (groupe "' .
			$groupes_joueurs[$element_lot['groupe_joueur']] . '") <br />
			Description du lot sur le site : ' . $element_lot['decription_lot'] . '<br />
			Description du lot dans l\'e-mail : ' . $element_lot['txt_mail_lot'] . '<br />';


			// Pour sauvegarde dans DB
			$rapport_1_concours_rec_db.= '<br />[Lot ' . $i_lot . '] (groupe "' .
			$groupes_joueurs[$element_lot['groupe_joueur']] . '") <br />
			Description du lot sur le site :' . $element_lot['decription_lot'] . '<br />
			Description du lot dans l\'e-mail : ' . $element_lot['txt_mail_lot'] . '<br />';
			...
			if ($liste_joueurs_lot_en_cours != NULL)
			{
				...
				pour ' . $nombre_places_dispo . ' unité(s) disponible(s)<br />';
	
	
				// Pour sauvegarde dans DB
				$rapport_1_concours_rec_db.= ' - ' . $nombre_participants . ' participant(s) 
				pour ' . $nombre_places_dispo . ' unité(s) disponible(s)  ______<br /> <br />';
				...
				foreach ($id_gagnants as $liloolou)
				{
					...
					//++++++++++++++++++++++++++++++++++++++++++++++
					// Coordonnées lieu pour que le gagnant puisse appeler le LIEU et confirmer sa venue (ajouté le 27/03/08)

					// Le concours est-il lié à un EVENEMENT de DLP ?
					if ($donnees['event_dlp_conc'] != 0)
					{
						$event_dlp_conc = $donnees['event_dlp_conc'] ;
						
						// trouver EVENT à partir de FICHE
						$reponse_sur_event = mysql_query("SELECT lieu_event FROM $table_evenements_agenda 
						WHERE id_event = '$event_dlp_conc'") ;
						$donnees_sur_event = mysql_fetch_array($reponse_sur_event);
						$donnees_sur_event_LIEU = $donnees_sur_event['lieu_event'] ;
						
						// trouver LIEU à partir de EVENT
						$reponse_sur_event_contact = mysql_query("SELECT e_mail_lieu, tel_lieu FROM $table_lieu 
						WHERE id_lieu = '$donnees_sur_event_LIEU'") ;
						$donnees_sur_event_contact = mysql_fetch_array($reponse_sur_event_contact);
						
						$liloolou_contacter = 'Téléphone : ' . $donnees_sur_event_contact['tel_lieu'] . ' <br />
						Email : <a href="mailto:' . $donnees_sur_event_contact['e_mail_lieu'] . '">' . $donnees_sur_event_contact['e_mail_lieu'] . '</a>' ;
					
					}
					elseif ($donnees['lien_externe_conc'] != NULL)
					{
						// Le concours est-il lié à un LIEN EXTERIEUR ?
						$liloolou_contacter = 'Vous trouverez les coordonnées via <a href="' . $donnees ['lien_externe_conc'] . '">ce lien</a> ';
					}
					...
					$test_envoi_joueur = informer_joueur($id_liloolou, $nom_liloolou, $mail_liloolou, $time_liloolou, $est_gagnant, $liloolou_contacter) ; 
					...
					else
					{
						...
						$rapport_1_concours_rec_db.= '<strong>ERREUR envoi email à  ' . $id_liloolou . ' !</strong> <br />';
					}
				}
				...
				// Mise en DB des ID des gagnants pour leur envoyer le rappel (ajouté en juin 2011)
				// rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr

				foreach ($id_gagnants as $gagnant_pour_db)
				{
					$reponse_info_joueur_2 = mysql_query("SELECT * FROM $table_ag_conc_joueur 
					WHERE id_conc_joueur = '$gagnant_pour_db'");
					$donnees_info_joueur_2 = mysql_fetch_array($reponse_info_joueur_2);
					
					$id_conc_joueur = $donnees_info_joueur_2['id_conc_joueur'] ;
					$id_fiche_conc_joueur = $donnees_info_joueur_2['id_fiche_conc_joueur'] ;
					/*echo '<p>id_conc_joueur : ' . $id_conc_joueur . '<br>
					id_fiche_conc_joueur : ' . $id_fiche_conc_joueur . '</p>' ;*/
					
					// Quelle est la date de cloture du concours ?
					$reponse_date_cloture_pour_avis = mysql_query("SELECT * FROM $table_ag_conc_fiches 
					WHERE id_conc = '$id_fiche_conc_joueur'");
					$donnees_date_cloture_pour_avis = mysql_fetch_array($reponse_date_cloture_pour_avis);
					
					$cloture_conc_pour_avis = $donnees_date_cloture_pour_avis['cloture_conc'] ;
					//echo '<p>Date : ' . $cloture_conc_pour_avis . '</p>' ;
										 
	
					mysql_query("INSERT INTO ag_conc_avis (`id_conc_avis`, `id_joueur`, `cloture_conc`) 
					VALUES ('', $id_conc_joueur, $cloture_conc_pour_avis)");
		...
		// IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
		// Informer le LIEU qui propose ces lots
		// IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
