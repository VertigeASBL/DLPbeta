...
if (confirm("Voulez-vous vraiment créer un nouvel concours ?")) {
	window.location.href='conc_2_listing.php?new=creer';
	}
}

function confirmation_effacer(ma_var) {
if (confirm("Voulez-vous vraiment effacer ce concours ?")) {
	window.location.href='conc_2_listing.php?effacer='+ma_var;
	}
}
...
<a href="../concours/conc_2_tirage.php?pw=s5fah7r6s3p6ax2">Tirage du concours</a></div>

<p align="center" class="mini">Les concours &quot;actifs&quot; sont les concours qui sont accessibles au public. Leur nom apparait en vert. 
Les concours qui sont clotur&eacute;s apparaissent dans une case fonc&eacute;e. Sélectionné : *</p>
...
if (isset($_GET['effacer']) AND ($_GET['effacer'] != NULL))
{
	...
	{		
		...
		<a href="conc_2_listing.php">Retour</a></p></div><br>' ;
...
{
	$date_annee_defaut = date(mktime(0, 0, 0, 1, 1, (date('Y')+0))) ;
	
	$new_conc = mysql_query("INSERT INTO `$table_ag_conc_fiches` (`nom_event_conc`, `cloture_conc`) 
	VALUES ('- -NOUVEAU CONCOURS- -', '$date_annee_defaut')")
	...
	<a href="conc_2_edit_a.php?id_conc='.$nouvel_id_table_concours.'">éditée</a></p></div><br>' ;
...
while ($donnees = mysql_fetch_array($reponse))
{
	...
	$tab.= '<br /><div class="' . $actif_ou_pas . '"><b>' . $nom_event_conc . '</b></div>'.($donnees['selection_conc'] ? '*' : ' ').'<br />';
	
	$tab.= 'Clôturé le ' . $date_cloture_jour . '-' . $date_cloture_mois . '-' . $date_cloture_annee . ' 
	à ' . $date_cloture_heure . 	'h00<br /><a href="conc_2_edit_a.php?id_conc=' . $id_conc . '">Editer</a>';
	...
	if (isset($donnees['lots_conc']) AND ($donnees['lots_conc'] != NULL))
	{
		...
		{
			$tab.= '<li><b>[lot ' . $i_lot . ']</b> : <b>' . str_pad($element_lot['nombre_places'], 3, "0", STR_PAD_LEFT) . 
			'</b> unités à remporter ';
			$tab.= 'pour le groupe "<b>' . $groupes_joueurs[$element_lot['groupe_joueur']] . '</b>"';
			$tab.= '<br /> <em>' . $element_lot['decription_lot'] . '</em> <br /> ' ;
	...
	else
	{
		...
		<a href="conc_2_edit_a.php?id_conc=' . $donnees ['id_conc'] . '">Editer</a></div>';
	}
