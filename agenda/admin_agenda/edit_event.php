...
if (isset($_GET['term'])) {
	...
	$result = array();
	$reponse = mysql_query('SELECT id_pres,nom_pres FROM ag_representation WHERE nom_pres LIKE \'%'.$term.'%\' ORDER BY nom_pres');
...
if (isset($_GET['id_pres'])) {
	...
	$reponse = mysql_query('SELECT lieu_pres,nom_pres,adresse_pres,localite_pres,postal_pres,pays_pres,ok_pres,nom_lieu FROM ag_representation LEFT JOIN ag_lieux ON id_lieu=lieu_pres WHERE id_pres='.$id_pres);
	if ($donnees = mysql_fetch_array($reponse))
		echo json_encode(array('lieu_pres'=>$donnees['lieu_pres'], 'nom_pres'=>utf8_encode($donnees['nom_pres']), 'adresse_pres'=>utf8_encode($donnees['adresse_pres']), 'localite_pres'=>utf8_encode($donnees['localite_pres']), 'postal_pres'=>utf8_encode($donnees['postal_pres']), 'pays_pres'=>$donnees['pays_pres'], 'ok_pres'=>$donnees['ok_pres'], 'nom_lieu_pres'=>utf8_encode(str_replace('&#039;', '\'', html_entity_decode($donnees['nom_lieu'])))));
	exit();
}
...
$(function() {
	...
	$("#nom_pres").autocomplete({
		source: "edit_event.php"
		, minLength: 2
		, search: function(event, ui) {
			if ($("#ok_nom_pres").attr('checked'))
				return;
			$("#id_pres_event").val(0);
			$("#lieu_pres").val(0);
			$("#adresse_pres").val("");
		...
		, close: function(event, ui) { //--- si on ferme sans sélectionner, essayer de trouver le lieu
			$.getJSON("edit_event.php?term="+encodeURI($("#nom_pres").val()), function(json){
				if ($("#ok_nom_pres").attr('checked'))
					return;
				if (json.length == 1) {
					$("#id_pres_event").val(json[0].id);
					$.getJSON("edit_event.php?id_pres="+json[0].id, function(json){
						if (json) {
							$("#nom_pres").val(json.nom_pres);
							$("#lieu_pres").val(json.lieu_pres);
							...
							$("#ok_pres").attr('checked', json.ok_pres!=0);
							$("#nom_lieu_pres").text(json.nom_lieu_pres);
						}
					});
				}
				else {
	...
	if (id_pres != 0)
		$.getJSON("edit_event.php?id_pres="+id_pres, function(json){
			$("#nom_pres").val(json.nom_pres);
			$("#lieu_pres").val(json.lieu_pres);
			$("#adresse_pres").val(json.adresse_pres);
			$("#localite_pres").val(json.localite_pres);
...
$id = (int) $_GET['id'];
	...
	$lieu_pres = isset($_POST['lieu_pres']) ? (int) $_POST['lieu_pres'] : 0;
	$nom_pres = isset($_POST['nom_pres']) && $_POST['nom_pres'] ? mysql_real_escape_string(str_replace('’', '\'', $_POST['nom_pres'])) : '';
	$adresse_pres = isset($_POST['adresse_pres']) && $_POST['adresse_pres'] ? mysql_real_escape_string(str_replace('’', '\'', $_POST['adresse_pres'])) : '';
	$localite_pres = isset($_POST['localite_pres']) && $_POST['localite_pres'] ? mysql_real_escape_string(str_replace('’', '\'', $_POST['localite_pres'])) : '';
	$postal_pres = isset($_POST['postal_pres']) && $_POST['postal_pres'] ? mysql_real_escape_string(str_replace('’', '\'', $_POST['postal_pres'])) : '';
	$pays_pres = isset($_POST['pays_pres']) && $_POST['pays_pres'] ? (int) $_POST['pays_pres'] : 1;
	$ok_pres = isset($_POST['ok_pres']) ? 1 : 0;

	$id_pres_event = isset($_POST['id_pres_event']) ? (int) $_POST['id_pres_event'] : 0;
	if ($id_pres_event) {
		if ($nom_pres)
			if ($adresse_pres || $localite_pres || $postal_pres)
				mysql_query("UPDATE ag_representation SET lieu_pres=$lieu_pres,nom_pres='$nom_pres',adresse_pres='$adresse_pres',localite_pres='$localite_pres',postal_pres='$postal_pres',pays_pres=$pays_pres,ok_pres=$ok_pres WHERE id_pres=$id_pres_event");
			else
				mysql_query("DELETE FROM ag_representation WHERE id_pres=$id_pres_event LIMIT 1");
	}
	else if ($nom_pres && $adresse_pres && $localite_pres && $postal_pres) {
		if (! $lieu_pres)
			$lieu_pres = $lieu_event;
		if (mysql_query("INSERT INTO ag_representation SET lieu_pres=$lieu_pres,nom_pres='$nom_pres',adresse_pres='$adresse_pres',localite_pres='$localite_pres',postal_pres='$postal_pres',pays_pres=$pays_pres,ok_pres=$ok_pres"))
			$id_pres_event = mysql_insert_id();
	}
	mysql_query("UPDATE $table_evenements_agenda SET pres_event=$id_pres_event WHERE id_event=$id LIMIT 1");

	// -----------------------------------------
	// PARENT DE L'EVENEMENT 
	$parent_event = isset($_POST['parent_event']) ? (int) $_POST['parent_event'] : 0;
	mysql_query("UPDATE $table_evenements_agenda SET parent_event=$parent_event WHERE id_event=$id LIMIT 1");
...
/*	if (isset($_POST['checkbox_en_journee']) AND ($_POST['checkbox_en_journee'] == "en_journee"))
	...
	// -----------------------------------------
	// TEST DE LA CHRONIQUE
	if (isset($_POST['chronique_event']) AND preg_match('/[0-9]$/', $_POST['chronique_event'])) 
	{
		$chronique_event = htmlentities($_POST['chronique_event'], ENT_QUOTES);
		mysql_query("UPDATE `$table_evenements_agenda` SET `chronique_event` = '$chronique_event' WHERE `id_event` = '$id' LIMIT 1 ");
	}
...
*/
	...
	// -----------------------------------------
	// TEST DU LIEN VERS UN EVENEMENT D'UNE SAISON ANTERIEURE (faire cela apres TEST INTERVIEW, TEST CRITIQUE, TEST CHRONIQUE)
	if (isset($_POST['saison_preced_event']) AND preg_match('/[0-9]$/', $_POST['saison_preced_event'])) 
	{
		$saison_preced_event = (int) $_POST['saison_preced_event'];
		$reponse = '';
		if ($_POST['saison_preced_memo'] != $saison_preced_event) {
			require_once '../../squelettes/mes_fonctions.php';
			if (! isset($_POST['interview_event']) || ! $_POST['interview_event'])
				$reponse .= ',interview_event='.saisonprecedente($saison_preced_event, 'interview');
			if (! isset($_POST['critique_event']) || ! $_POST['critique_event'])
				$reponse .= ',critique_event='.saisonprecedente($saison_preced_event, 'critique');
			if (! isset($_POST['chronique_event']) || ! $_POST['chronique_event'])
				$reponse .= ',chronique_event='.saisonprecedente($saison_preced_event, 'chronique');
		}
		$saison_preced_event = htmlentities($saison_preced_event, ENT_QUOTES);
		mysql_query("UPDATE `$table_evenements_agenda` SET `saison_preced_event`='$saison_preced_event' $reponse WHERE `id_event`='$id' LIMIT 1");
	}
	...
	{
		echo '<div class="alerte"><p>Erreur ! Les données n\'ont pas été enregistrées</p></div><br>' ;
	}
		...
		*/	?>
          ...
          <tr>
			...
			while ($donnees_2 = mysql_fetch_array($reponse_2))
			{
				...
				$max=70; // Longueur MAX de la cha&icirc;ne de caract&egrave;res
          ...
          <tr>
            ...
            <td><input name="nom_event" type="text" id="nom_event" value="<?php if (isset($nom_event)){echo htmlspecialchars($nom_event);} ?>" size="70" maxlength="200"></td>
          </tr>
...
?>
	...
	echo '<tr>',"\n",'<td colspan="2">',"\n",'<span class="mini">Entrez quelques lettres ci-dessous pour sélectionner un lieu de représentation existant. Ou remplissez les données complètes d\'un nouveau lieu de représentation (nom, adresse, localité, code postal, pays). Quand on ajoute un nouveau lieu de représentation, on peut laisser à 0 le producteur lié, alors le producteur lié sera le producteur de l\'événement. Pour supprimer le lieu de représentation, il faut laisser le nom du lieu et vider adresse, localité, code postal. Cochez la case pour pouvoir éditer le nom.</span>',"\n",'</td>',"\n",'</tr>',"\n";
	echo '<tr>',"\n",'<td>Nom du lieu de représentation',"\n";
	echo '<input type="hidden" name="id_pres_event" id="id_pres_event" value="',$id_pres_event,'" />',"\n",'</td>',"\n";
	echo '<td class="ui-widget">',"\n",'<input type="text" id="nom_pres" name="nom_pres" value="" size="60" /> <input type="checkbox" id="ok_nom_pres" value="1" />',"\n",'</td>',"\n",'</tr>',"\n";

	echo '<tr>',"\n",'<td>Validé',"\n",'</td>',"\n";
	echo '<td>',"\n",'<input type="checkbox" name="ok_pres" id="ok_pres" value="1" />',"\n",'</td>',"\n",'</tr>',"\n";

	echo '<tr>',"\n",'<td>Producteur lié au lieu',"\n",'</td>',"\n";
	echo '<td>',"\n",'<input type="text" name="lieu_pres" id="lieu_pres" value="" size="8" /> <span id="nom_lieu_pres"></span>',"\n",'</td>',"\n",'</tr>',"\n";
...
<?php
	echo '<input name="saison_preced_event" type="text" id="saison_preced_event" value="',(isset($saison_preced_event) && $saison_preced_event!=0 ? $saison_preced_event : ''),'" size="5" maxlength="8" /> <input name="saison_preced_memo" type="hidden" value="',$saison_preced_event,'" />';
?>
		...
		{echo $jai_vu_event;}?>" size="5" maxlength="8" disabled="disabled" />
            </td>
          </tr>
...
<input name="interview_event" type="text" id="interview_event" value="<?php 
		...
		{echo $interview_event;}?>" size="5" maxlength="5">
		  ...
		  <tr>
            <td>Chronique li&eacute;e </td>
            <td>indiquer un num&eacute;ro d'article SPIP
              :
              <input name="chronique_event" type="text" id="chronique_event" value="<?php 
			if (isset ($chronique_event) AND $chronique_event != 0)
		{echo $chronique_event;}?>" size="5" maxlength="5">
              (z&eacute;ro pour effacer) </td>
          </tr>


		  <tr>
            <td>Lien vers espace livres</td>
            <td>indiquer un num&eacute;ro d'article SPIP
              :
		...
		{echo $video_spip_event;}?>" size="5" maxlength="5">
          ...
          <tr>
			 ...
			 if (isset ($donnees ['video_event']) AND $donnees ['video_event'] != NULL )
			{
				...
				title="Voir la vidéo liée à cet événement" /><img src="../design_pics/ico_video.jpg" alt="" /></a>';
			}
