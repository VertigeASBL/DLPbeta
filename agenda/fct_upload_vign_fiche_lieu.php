...
{		
	...
	$destination = '../' . $folder_pics_vignettes_lieux_culturels . 'pic_fiche_lieu_' . $id_update . '_' . $num_pic . '.jpg' ; // Chemin & nom de image de destination
	$destination_vi = '../' . $folder_pics_vignettes_lieux_culturels . 'vignette_fiche_lieu_' . $id_update . '_' . $num_pic . '.jpg' ; // Chemin & nom de Vignette
	...
	else 
	{
		...
		{
			chmod ($destination, 0777); // Pour que l'image ait un CHMOD 777 et non 600 
			...
			if ($largeur_uploaded<$maxWidth_pic AND $hauteur_uploaded<$maxHeight_pic) // H > maximum et W > maximum. On cherche la plus grande valeur ==> Inutile de redimentionner l'image
			{	
			...
			else  // H > maximum ET/OU W > maximum ==> Il faut redimentionner l'image
			{
				...
				if ($largeur_uploaded>$maxWidth_pic AND $hauteur_uploaded>$maxHeight_pic) // H > maximum et W > maximum. On cherche la plus grande valeur
				{
					...
					if ($hauteur_uploaded>$largeur_uploaded) 
					{
						...
						$new_W = round($largeur_uploaded * $maxHeight_pic / $hauteur_uploaded);
					...
					else // La largeur est plus grande
					{
						...
						$new_H = round($hauteur_uploaded * $maxWidth_pic / $largeur_uploaded);
						$debug_concat.=  '<br />- <b>nouvelle hauteur</b> = '.$new_H ;
						$debug_concat.=  '<br />- <b>nouvelle largeur</b> = '.$new_W ;
					}
				}
				else if ($largeur_uploaded<$maxWidth_pic AND $hauteur_uploaded>$maxHeight_pic) // H > maximum et W < maximum. On cherche la plus grande valeur
			...
			if ($largeur_uploaded<$maxWidth AND $hauteur_uploaded<$maxHeight)
			{	
			...
			else
			{
				...
				if ($largeur_uploaded>$maxWidth AND $hauteur_uploaded>$maxHeight)
				{
					if ($hauteur_uploaded>$largeur_uploaded) 
					{
						...
						// La hauteur est plus grande
						$new_H_Vignette = $maxHeight;
						// On recalcule la Largeur proportionnellement
						$new_W_Vignette = round($largeur_uploaded * $maxHeight / $hauteur_uploaded);
					...
					else
					{
						...
						$new_W_Vignette = $maxWidth;
						// On recalcule la Hauteur proportionnellement
						$new_H_Vignette = round($hauteur_uploaded * $maxWidth / $largeur_uploaded);
						$debug_concat.=  '<br /><b>- nouvelle hauteur Vignette</b> = '.$new_H_Vignette ;
						$debug_concat.=  '<br /><b>- nouvelle largeur Vignette</b> = '.$new_W_Vignette ;
					}
				}
				else if ($largeur_uploaded<$maxWidth AND $hauteur_uploaded>$maxHeight)
				{
			...
			mysql_query("UPDATE `$table_lieu` SET `pic_lieu` = 'set' WHERE `id_lieu` = '$id_update' LIMIT 1 ");
		}			
