...
// ******************************************************
	// date de début
	$date_in_aaammjj = date ('Y-m-d', mktime(0, 0, 0, date("m"), date("d")-15, date("Y"))); 
...
{		//$chaine_txt_libre
	...
	while ($donnees_titres_events = mysql_fetch_array($reponse_titres_events))
	{
		...
		$nom_event_list_echap = str_replace("'", "\'",$nom_event_list); // Modifié le 30-11-2010
		$liste_titre_compose.= '<li onClick="fill(\'' . $nom_event_list_echap . '\');">' . $nom_event_list . '</li>';
	}
