...
/* Afin de pouvoir ordonner le résultat de la recherche (pas de répétition de LIEU...) on récolte les ID données par la requête, on les réordonne, puis on les stocke dans un Array qui sera mis en SESSION et lu à chaque demande de pagination. cfrr "Début du Tri" */
...
else
{
	...
	*/
	...
	if (isset($_GET['req']) AND $_GET['req'] == 'mini_calendr')
	{
		...
		// Pour le tri par titre
		$requete_concat_ordre_titre.= " AND jours_actifs_event LIKE '%$date_a_tester_dans_jours_actifs%' " ;
	}
	...
	if (isset($genre_event) AND $genre_event != '')
	{
		...
		// Pour le tri par titre
		$requete_concat_ordre_titre.= " AND genre_event = '$genre_event' " ;
	...
	if (isset($lieu_event) AND $lieu_event != '')
	{
		...
		// Pour le tri par titre
		$requete_concat_ordre_titre.= " AND lieu_event = '$lieu_event' " ;
	...
	if (isset($ville_event) AND $ville_event != '')
	{
		...
		// Pour le tri par titre
		$requete_concat_ordre_titre.= " AND ville_event = '$ville_event' " ;
...
/*	// page_aff  
	...
	*/
	...
	if (isset($chp_txt_libre) AND $chp_txt_libre != '')
	{
		$requete_add_ordre_titre= " AND (nom_event LIKE '%$chp_txt_libre_echap%') ";
...
exit();	*/
	...
	/*echo '<em> '.$cpt_bcl.') </em><pre>';
	print_r($array_id_selection_tout);
	echo '</pre>';*/
	...
	{	
		...
		{
			/* Cet événement provoque une suite consécutive de Lieu ! Il est refusé (donc mis dans Array Temp. 
			On vérifie quand même si on peut repêcher un Lieu dela Table Temporaire */
	...
	/*	echo '<em> '.$cpt_bcl.') </em><pre>';
		print_r($array_id_doublons_temp);
		echo '</pre>';*/
...
} // Fin du "saut de la requête SQL" effectué lorsqu'un saut de pagination est effectué mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
...
//--- richir	$tab.= '<a href="-Interviews-?id_article=' . $interview_event . '&amp;page=article-3" title...
	...
	$reponse_synchrone_2 = mysql_query("SELECT * FROM $table_lieu WHERE id_lieu = $id_lieu");
	$donnees_synchrone_2 = mysql_fetch_array($reponse_synchrone_2) ;
