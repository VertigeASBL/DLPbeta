...
function transformFilePath($value) {
	...
	global $conf_sys_root_path ;	// stratmodif

	$rootPath = backslashToSlash(addTrailingSlash(realpath($conf_sys_root_path)));
...
function addNoCacheHeaders() {
	...
	function isUnderRoot($value)
	{
		...
		$roorPath = strtolower(backslashToSlash(addTrailingSlash($conf_sys_root_path)));
