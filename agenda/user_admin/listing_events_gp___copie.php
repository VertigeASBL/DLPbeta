...
{
	$tab.= '<div class="alerte"><br />Par le passé, vous avez créé des "fiches événements" que vous n\'avez pas remplies pour la raison que vous avez interrompu la procédure en cours de route. Nous vous demandons d\'utiliser prioritairement les événements listés ci-dessous, et de ne pas créer de nouvel événement tant qu\'il reste des éléments dans cette liste. Cliquez simplement sur un élément de la liste et vous pourrez l\'éditer.<br /><br />';
	
	while ($donnees_des_vides = mysql_fetch_array($reponse_des_vides))
	{
		$tab.= '<a href="edit_event_gp_tiny.php?id='. $donnees_des_vides['id_event'] . '">Evénement '. $donnees_des_vides['id_event'] . '</a> <br />';
	}
	$tab.= '</div><br /><br /><br />';
}
...
$reponse = mysql_query("SELECT	A.id_event,A.parent_event,A.nom_event,A.date_event_debut,A.date_event_fin,A.pic_event_1, IF(A.parent_event=0,A.date_event_debut,B.date_event_debut) AS tridate
...
for ($num = 0; $donnees = mysql_fetch_array($reponse); $num++) {
	// ____________________________________________
	// ID
	if ($donnees = mysql_fetch_array($reponse))
		$parent = $donnees['parent_event'];
	else
		$parent = 0;
	mysql_data_seek($num);
	$donnees = mysql_fetch_array($reponse);
	$tab.= '<tr class="tr_hover">'.($parent ? '<td class="evparent">' : ($donnees['parent_event'] ? '<td class="evenfant">' : '<td>')).'<i>'.$donnees['id_event'].'/'.$donnees['parent_event'].'</i></td>' ;
