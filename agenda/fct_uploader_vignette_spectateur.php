...
// Upload image et vignette pour les FICHES SPECTATEURS
...
function uploader_vignette_spect ($id_update,$num_pic)
{		
	...
	else 
	{
		if(move_uploaded_file($_FILES[$fichier_source]['tmp_name'], $destination)) // Copie de l'image sur le serveur
		{
			...
			if ($largeur_uploaded >= $w_absolue) // Largeur Suffisante
			{
				...
				//$image_db = 'pic_spectateur_' .  $num_pic ;
				mysql_query("UPDATE $table_spectateurs_ag SET pic_spectateur = 'set' WHERE id_spectateur = '$id_update' LIMIT 1 ");
			}
			else
			{
				...
				//$image_db = 'pic_spectateur_' .  $num_pic ;
				mysql_query("UPDATE $table_spectateurs_ag SET pic_spectateur = '' WHERE id_spectateur = '$id_update' LIMIT 1 ");
