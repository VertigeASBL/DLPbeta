...
// Cett version de fonction n'est plus utilisée car remplacée par "fct_upload_pic_event_3.php" 
// L'image et la vignette sont redimenstionnées en W ET en H selon les valeurs MAX prédéfinies
// FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
...
{		
	...
	else 
	{
		if(move_uploaded_file($_FILES[$fichier_source]['tmp_name'], $destination)) // Copie de l'image sur le serveur
		{
			...
			if ($largeur_uploaded<$max_w_pic_event AND $hauteur_uploaded<$max_h_pic_event) // H > maximum et W > maximum. On cherche la plus grande valeur ==> Inutile de redimentionner l'image
			{	
			...
			else  // H > maximum ET/OU W > maximum ==> Il faut redimentionner l'image
			{
				...
				if ($largeur_uploaded>$max_w_pic_event AND $hauteur_uploaded>$max_h_pic_event) // H > maximum et W > maximum. On cherche la plus grande valeur
				{
					// La hauteur est plus grande
					if ($hauteur_uploaded>$largeur_uploaded) 
					{
						$debug_concat.=  '<br />- Resamp:::1:::';
						$new_H = $max_h_pic_event;
						// On recalcule la Largeur proportionnellement
						$new_W = round($largeur_uploaded * $max_h_pic_event / $hauteur_uploaded);
					...
					else // La largeur est plus grande
					{
						...
						$new_W = $max_w_pic_event;
						// On recalcule la Hauteur proportionnellement
						$new_H = round($hauteur_uploaded * $max_w_pic_event / $largeur_uploaded);
						$debug_concat.=  '<br />- <b>nouvelle hauteur</b> = '.$new_H ;
						$debug_concat.=  '<br />- <b>nouvelle largeur</b> = '.$new_W ;
					}
				}
				else if ($largeur_uploaded<$max_w_pic_event AND $hauteur_uploaded>$max_h_pic_event) // H > maximum et W < maximum. On cherche la plus grande valeur
