...
function affich_jours_spectacles ($MM_traite, $AAAA_traite)
{
	...
	$next = '?req=mini_calendr&date_debut=01-' . $mois_next . '-' . $annee_next . '&date_fin=' . $valeur_dernier_jour_mois . '-' . $mois_next . '-' . $annee_next ;
	...
	{ 
		...
		for ($j=1 ; $j<=31 ; $j++)
		{
			...
			if (in_array($date_traite, $jours_actifs_event))
			{
				...
				$MM_traite = str_pad($MM_traite, 2, "0", STR_PAD_LEFT) ;  // Complète la chaîne
				$link = generer_url_entite(65, 'rubrique').'?req=mini_calendr&date_debut=' . $j . '-' . $MM_traite . '-' . $AAAA_traite . '&date_fin=' . $j . '-' . $MM_traite . '-' . $AAAA_traite ;
...
if (! preg_match('/^\d{2}\D\d{2}\D\d{4}$/', $date_debut))
	$date_debut = date('d-m-Y');
affich_jours_spectacles (substr($date_debut, 3, 2), substr($date_debut, 6, 4)) ;
?>